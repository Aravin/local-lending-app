...
class GetBorrowerLoans {
  const GetBorrowerLoans(this._repository);

  final LoanRepository _repository;

  Future<Either<Failure, List<Loan>>> call(String borrowerId) {
    if (borrowerId.trim().isEmpty) {
      return Future.value(
        const Left(ValidationFailure('Borrower id is required.')),
      );
    }
    return _repository.getBorrowerLoans(borrowerId);
  }
}
